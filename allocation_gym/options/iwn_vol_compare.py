"""
IWN options pricing comparison: this week vs last week.

Fetches recent IWN data, computes realized vol for both windows,
prices a grid of calls/puts at each vol level, and shows the delta.

Usage:
    python -m allocation_gym.options.iwn_vol_compare
"""
...
def load_iwn(days: int = 500) -> pd.DataFrame:
    """Load IWN data and filter for price < $200"""
    ...
    # Filter for price < $200
    df = df[df["Close"] < 200.0]

    if df.empty:
        raise RuntimeError("No IWN data with price < $200 found")
...
def run_comparison():
    print("=" * 70)
    print("IWN OPTIONS PRICING: THIS WEEK vs LAST WEEK (Price < $200)")
    print("=" * 70)

    # Load data
    print("\nLoading IWN data (filtering for price < $200)...")
    df = load_iwn(days=500)
    ...
    two_week = df.iloc[-10:]
    month = df.iloc[-21:] if len(df) >= 21 else df
    ...
    print(f"  IWN price moved: ${S_last_week:.2f} -> ${S_now:.2f} ({price_chg:+.2f}%)")
    if rv_this > rv_last:
...
def _plot_vol_comparison(
    ...
    fig.suptitle(
        f"IWN OPTIONS: This Week vs Last Week (Price < $200)  |  "
    ...
    # ── Panel 1: IWN price + vol shading ─────────────────────────────
    ax = axes[0, 0]
    dates = df.index
    ax.plot(dates, df["Close"], color="#333", linewidth=1.5, label="IWN Close")
    ...
    ax.set_title("IWN Price + Weekly Windows", fontsize=11, fontweight="bold")
    ...
    out_path = os.path.join(out_dir, "iwn_vol_compare.png")
    fig.savefig(out_path, dpi=150, bbox_inches="tight", facecolor="white")
    print(f"Saved to {out_path}")

    # plt.show(block=True)  # Disabled for non-interactive use
